...
if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) {
    Add-Issue -List $errors -Code 'FILE_NOT_FOUND' -Line 0 -Message "Output file does not exist: $Path"
}
else {
    ...
    for ($index = 0; $index -lt $lines.Count; $index++) {
        ...
        foreach ($item in $ordered) {
            if ($itemText.StartsWith($item.Glyph, [System.StringComparison]::Ordinal)) {
                Add-Issue -List $errors -Code 'SYMBOL_ON_LIST_ITEM' -Line ($index + 1) -Message 'Semantic symbols belong in section headings, not repeated list items.'
                break
            }
        }
    }
    ...
    if ($sections.Count -eq 0) {
        Add-Issue -List $errors -Code 'NO_SECTIONS' -Line 0 -Message 'No semantic sections were found.'
    }
    else {
